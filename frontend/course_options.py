...
def render_skill_course_options(conn, skill, study_hours_per_week, top_n, sort_by, max_cost, max_duration, difficulties, provider_keyword, compact=False, context_key="default",):
    ...
    if len(filtered) > top_n:
        with st.expander(f"See full list for {skill_name} ({len(filtered):,} courses)"):
            ...
            st.markdown("#### Select from full list")

            course_options = [
                f"{idx + 1}. {row['title']} — {row['provider']} (${row['cost']:.0f}, {row['duration_hours']:.0f} hrs)"
            ...
            selected_label = st.selectbox(
                ...
                course_options,
                key=f"full_list_select_{context_key}_{skill_name}",
            )

            selected_idx = course_options.index(selected_label)
            selected_row = filtered.iloc[selected_idx]

            if st.button(
                "Add selected course to plan",
                key=f"full_list_add_{context_key}_{skill_name}",
            ):
                add_selected_course(
                    skill_name,
...
def render_course_options_explorer(conn, missing_skills, study_hours_per_week, title="Recommended course options", compact=False, show_selected_plan=True, context_key="default",):
    ...
    For each skill:
    - explore multiple real SkillsFuture course options
    - compare by cost, duration, and difficulty
    - select courses to add into your personalised learning plan

    💡 The top course options are recommended using the current filter and sorting settings.
    """)
...
def render_plan_comparison(route_summaries, study_hours_per_week: float):
    selected_df = get_selected_courses_df()

    st.markdown("### 📊 Plan Comparison")
    st.caption(
        "Recommended strategies are fixed system benchmarks. Your plan is based on the courses you personally selected."
    )
    ...
    for route_name, summary in route_summaries.items():
        ...
        rows.append({
            ...
            "Time": f"{summary['total_months']} months",
            "Cost": f"${summary['total_cost']:,.0f}",
            "Difficulty": f"{summary['avg_difficulty']:.1f}/5",
            "Notes": "Fixed route generated by the system",
        })
    ...
    if not selected_df.empty:
        ...
        rows.append({
            "Plan": "Your custom plan",
            "Type": "User selected",
            "Time": f"{estimated_weeks:.1f} weeks / {estimated_months:.1f} months",
            "Cost": f"${total_cost:,.0f}",
            "Difficulty": "Based on selected courses",
            "Notes": f"{len(selected_df)} courses selected across {selected_df['skill'].nunique()} skills",
        })
    else:
        rows.append({
            "Plan": "Your custom plan",
            ...
            "Time": "Not built yet",
            "Cost": "Not built yet",
            "Difficulty": "Not built yet",
            "Notes": "Select courses below to build your plan",
        })
    ...
    if selected_df.empty:
        st.info("Select courses below to compare your custom plan against the system-generated strategies.")
        return

    fastest = route_summaries.get("Fastest route")
    cheapest = route_summaries.get("Cheapest route")

    if fastest:
        total_hours = selected_df["duration_hours"].sum(skipna=True)
        your_months = (total_hours / study_hours_per_week) / 4.345 if study_hours_per_week else 0
        your_cost = selected_df["cost"].sum(skipna=True)

        insight_parts = []

        if your_months < fastest["total_months"]:
            insight_parts.append("Your selected course plan is faster than the fastest benchmark route.")
        else:
            insight_parts.append("Your selected course plan takes longer than the fastest benchmark route.")

        if cheapest and your_cost < cheapest["total_cost"]:
            insight_parts.append("Your selected course plan is cheaper than the cheapest benchmark route.")
        elif cheapest:
            insight_parts.append("Your selected course plan costs more than the cheapest benchmark route.")

        st.success(" ".join(insight_parts))